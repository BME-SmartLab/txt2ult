...
dir_ref = 'experiments/' + speaker + '/acoustic_model/inter_module/ref_data/'
dir_gen = 'experiments/' + speaker + '/acoustic_model/gen/rec_4_tanh_1_lstm_512_adam_30_warmup/'
...
if os.path.isdir(dir_ref):
    for file in sorted(os.listdir(dir_ref)):
        if file.endswith('.ultpca128'):
            ult_files_ref += [dir_ref + file[:-10]]
...
# for basefilename in ult_files_ref[0:1] + ult_files_gen[0:1]:
# for basefilename in []:
for basefilename in ult_files_ref + ult_files_gen:
...
for i in range(len(ult_files_ref)):
    
    vidfile_ref = ult_files_ref[i] + '_ultrasound_ustools.mp4'
