...
# recurrent / LSTM
$SED -i s#'hidden_layer_size\s*:.*'#'hidden_layer_size:  [1024, 1024, 1024, 1024, 512]'# $duration_config_file
$SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH'\'', '\''TANH'\'', '\''TANH'\'', '\''TANH'\'', '\''LSTM'\'']'# $duration_config_file
$SED -i s#'model_file_name\s*:.*'#'model_file_name: rec_4_tanh_1_lstm_512_adam_30_warmup'# $duration_config_file

$SED -i s#'sequential_training\s*:.*'#'sequential_training: True'# $duration_config_file
$SED -i s#'warmup_epoch\s*:.*'#'warmup_epoch: 30'# $duration_config_file
$SED -i s#'training_epochs\s*:.*'#'training_epochs: 100'# $duration_config_file
$SED -i s#'optimizer\s*:.*'#'optimizer: adam'# $duration_config_file
...
# in_lf0_dir: %(data)s/lf0  -> add ultpca128
$SED -i s#'in_lf0_dir:.*'#'in_lf0_dir: %(data)s/lf0\nin_ultpca128_dir: %(data)s/ultpca128'# $acoustic_config_file
...
# add new lines for ultpca128
$SED -i s#'dlf0\s*:.*'#'dlf0: 3\nultpca128: 128\ndultpca128: 384'# $acoustic_config_file
...
# recurrent / LSTM
$SED -i s#'hidden_layer_size\s*:.*'#'hidden_layer_size:  [1024, 1024, 1024, 1024, 512]'# $acoustic_config_file
$SED -i s#'hidden_layer_type\s*:.*'#'hidden_layer_type: ['\''TANH'\'', '\''TANH'\'', '\''TANH'\'', '\''TANH'\'', '\''LSTM'\'']'# $acoustic_config_file
$SED -i s#'model_file_name\s*:.*'#'model_file_name: rec_4_tanh_1_lstm_512_adam_30_warmup'# $acoustic_config_file

$SED -i s#'sequential_training\s*:.*'#'sequential_training: True'# $acoustic_config_file
$SED -i s#'warmup_epoch\s*:.*'#'warmup_epoch: 30'# $acoustic_config_file
$SED -i s#'training_epochs\s*:.*'#'training_epochs: 100'# $acoustic_config_file
$SED -i s#'optimizer\s*:.*'#'optimizer: adam'# $acoustic_config_file


# [Streams]
# output_features      : ['mgc', 'lf0', 'vuv', 'bap'] + 'ultpca128'
# gen_wav_features     : ['mgc', 'lf0', 'bap'] +  'ultpca128'
$SED -i s#"output_features\s*:.*"#"output_features      : ['mgc', 'lf0', 'vuv', 'bap', 'ultpca128']"# $acoustic_config_file
$SED -i s#"gen_wav_features\s*:.*"#"gen_wav_features     : ['mgc', 'lf0', 'bap', 'ultpca128']"# $acoustic_config_file
